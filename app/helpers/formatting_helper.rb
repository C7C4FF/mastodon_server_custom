...
module FormattingHelper
  ...
  def status_content_format(status)
    ...
    options = { preloaded_accounts: [status.account] + (status.respond_to?(:active_mentions) ? status.active_mentions.map(&:account) : []), quoted_status: quoted_status }

    return MarkdownStatusFormatter.new(status.text, options).to_s if status.local?

    html_aware_format(status.text, false, options)
  end
